...
const ApprovalPage = () => {
    ...
    const loading = recruitmentLoading;
    console.log('designation', user?.designation?.name);
    const currentFlow = recruitmentData?.data?.[0]?.approvalFlow || [];
    if (currentFlow?.length < 4) {
        approvalStatus = ['pending_hr', 'pending_ceo', 'approved'];
    }
    console.log('recruitmentData', recruitmentData, currentFlow);
    const handleApprove = async () => {
        ...
        try {
            ...
            toast.success(`Requisition request approved successfully.`);
            let emailData = {};
    ...
    return (
        <div className="h-screen flex items-center justify-center mx-2">
            <div className="w-[550px] bg-white shadow-lg rounded-lg p-6">
                {/* Details Section */}
                <p className='font-bold text-lg pb-4 flex justify-center items-center'>Manpower Requisition Details</p>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td className="px-3 py-4  border-gray-300 rounded-tr-md">
                                {recruitmentData?.data?.[0]?.requestedBy?.displayName?.toProperCase() || recruitmentData?.data?.[0]?.requestedBy?.firstName?.toProperCase()  || ""}
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td className="px-3 py-4 border-t border-b   border-gray-300">
                                {recruitmentData?.data?.[0]?.requestDate ? moment(recruitmentData?.data?.[0]?.requestDate).format("DD-MMM-yyyy hh:mm A") : '—'}
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td className="px-3 py-4  border-gray-300 ">
                                {recruitmentData?.data?.[0]?.department?.name || "—"}
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td className="px-3 py-4 border-t border-gray-300 rounded-br-md">
                                {recruitmentData?.data?.[0]?.requiredPosition?.name || "—"}
                            </td>
                        </tr>
                    </tbody>
                </table>
