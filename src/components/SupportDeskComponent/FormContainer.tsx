...
const FormContainer: React.FC<FormContainerProps> = ({ formConfig, onSubmit, initialData = {}, action, users }) => {
    ...
    return (
        <FormProvider {...methods}>
            ...
            >
                {/* 🔹 Fixed Header */}
                <div className="sticky top-0 z-20 bg-white border-b border-gray-200 pb-4 flex justify-between items-center">
                    <div className="font-bold text-xl">
                        {action} {formConfig?.title?.toProperCase()}
                    </div>

                    <div className="flex gap-2">
                        <Button
                            type="submit"
                            className={`px-3 ${(action !== 'Add') && 'hidden'}`}
