...
const TicketStatisticsComponent: React.FC<TicketStatisticsComponentProps> = ({
  ...
  console.log('Status by Priority:', statusByPriority);

  const statusByPriorityData = Object.keys(statusByPriority).map(status => ({
    name: status,
    normal: statusByPriority[status].NORMAL || statusByPriority[status].normal || 0,
    high: statusByPriority[status].HIGH || statusByPriority[status].high || 0,
    critical: statusByPriority[status].CRITICAL || statusByPriority[status].critical || 0,
  ...
  const PRIORITY_COLORS: any = {
    'CRITICAL': '#d55959', // Primary color for high priority
    'HIGH': '#FFB547',
    'NORMAL': '#4ADE80',
    'critical': '#d55959',
    'high': '#FFB547',
    ...
    >
      ...
      <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" variants={container}>
        ...
        <motion.div variants={item}>
          <Card className="overflow-hidden h-full border-border/30 shadow-sm hover:border-primary/30 hover:shadow-md transition-all duration-300 bg-gray-100">
            <CardContent className="p-0">
              <div className="flex h-full">
                <div className="w-2 min-h-[162px] bg-primary/50"></div>
        ...
        {/* Open Tickets */}
        <motion.div variants={item}>
          <Card className="overflow-hidden border-border/30 shadow-sm hover:border-blue-300/30 hover:shadow-md transition-all duration-300 bg-gray-100">
            <CardContent className="p-0">
              <div className="flex h-full">
                ...
                <div className="flex-1 p-5">
                  ...
                  <div className="mt-4 pt-2 border-t border-border/20">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium">{filteredTickets.filter(t => t.status === 'Pending').length}</span>
                    </div>
        ...
        {/* High Priority */}
        <motion.div variants={item}>
          <Card className="overflow-hidden border-border/30 shadow-sm hover:border-red-300/30 hover:shadow-md transition-all duration-300 bg-gray-100">
            <CardContent className="p-0">
              <div className="flex h-full">
                <div className="w-2 bg-red-300"></div>
                <div className="flex-1 p-5">
                  <div className="flex justify-between">
                    <div>
                  ...
                  <div className="flex gap-1 mt-4 text-sm">
                    <div className="flex-1 text-center p-1 rounded bg-red-50/50 border border-red-100">
                      ...
                      <div className="font-medium text-red-600">
                        {filteredTickets.filter(t => t.priority === 'high' && ['Pending', 'In Progress'].includes(t.status)).length}
                      </div>
                    </div>
                    <div className="flex-1 text-center p-1 rounded bg-green-50/50 border border-green-100">
                      <div className="text-xs text-muted-foreground">Resolved</div>
                      <div className="font-medium text-green-600">
                        {filteredTickets.filter(t => t.priority === 'high' && ['Completed', 'Closed'].includes(t.status)).length}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* Completed */}
        <motion.div variants={item}>
          <Card className="overflow-hidden h-full border-border/30 shadow-sm hover:border-emerald-300/30 hover:shadow-md transition-all duration-300 bg-gray-100">
            <CardContent className="p-0">
              <div className="flex min-h-[162px] h-full">
                <div className="w-2 bg-emerald-300"></div>
                <div className="flex-1 p-5">
                  <div className="flex justify-between">
                    <div>
      ...
      {/* Activity Trends */}
      <motion.div variants={item}>
        <Card className="overflow-hidden border-border/30 shadow-sm bg-gray-100">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Task activity over the last 30 days</CardDescription>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Status Distribution */}
        <motion.div variants={item}>
          <Card className="overflow-hidden border-border/30 shadow-sm h-full bg-gray-100">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                      paddingAngle={3}
                      label={({ name, value, percent }) => percent > 0 ? `${name} (${value})` : ''}
                    >
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                      paddingAngle={5}
                      label={({ name, value, percent }) => percent > 0.05 ? `${name?.toProperCase()} (${value})` : ''}
                    >
                ...
                {priorityData.map((item, index) => (
                  ...
                  >
                    ...
                    <div className='flex gap-2'>
                      <div className="text-sm text-muted-foreground">{item.name?.toProperCase()}</div>
        ...
        <motion.div variants={item}>
          <Card className="overflow-hidden border-border/30 shadow-sm h-full bg-gray-100">
            <CardHeader className="pb-2">
              ...
              <CardDescription>Detailed breakdown of task status grouped by priority</CardDescription>
            </CardHeader>
            <CardContent className="p-6">
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <motion.div variants={item}>
            <Card className="overflow-hidden border-border/30 shadow-sm h-full bg-gray-100">
              <CardHeader className="pb-2">
                ...
                <CardDescription>Tasks by department (top {Math.min(5, departmentData.length)})</CardDescription>
              </CardHeader>
              <CardContent className="p-6">
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
