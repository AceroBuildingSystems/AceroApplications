...
console.log("formType", formType)
  const initializeWorkflow = useCallback((workflowData: any) => {
    ...
    console.log('completed', completed, formsData)
    const initialSteps = template.steps.map((templateStep: any, index: number) => {
      ...
      return {
        ...
        formId: formsData[templateStep.formType] || savedStep?.formId || null,
      };
    });
  ...
  useEffect(() => {
    console.log('💾 WORKFLOW: Saving workflow data to session storage', {
      workflowId,
      workflowType,
      currentStepIndex,
      steps,
