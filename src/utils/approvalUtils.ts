...
/**
 * Check if a user has permission to approve at the current step
 */
export async function canUserApprove(
  ...
  if (!approvalInstance || approvalInstance.status !== 'Pending') {
    return false;
  }
  ...
  // Get the user with department and role info
  ...
  if (!user || !user.employmentDetails) {
    return false;
  }
  
  // Check if user is delegated to
  if (
    currentStep.delegatedTo && 
    currentStep.delegatedTo.toString() === userId
  ) {
    return true;
  }
  
  // Check by role (if step requires a specific role)
  if (
    currentStep.role && 
    user.employmentDetails.role && 
    currentStep.role.toString() === user.employmentDetails.role._id.toString()
  ) {
    return true;
  }
  
  // Check by department and role (if step is department specific)
  if (
    currentStep.department && 
    user.employmentDetails.department && 
    currentStep.department.toString() === user.employmentDetails.department._id.toString()
  ) {
    // For department approvals, check if user is a manager
    if (user.departmentRole === 'manager') {
      // Check if user is the department manager
      const department = await Department.findById(currentStep.department);
      if (department && department.manager && department.manager.toString() === userId) {
        return true;
      }
    }
  }
...
/**
 * Get all approval instances that a user needs to act on
 */
export async function getUserPendingApprovals(userId: string) {
  await connectToDB();
  ...
  const userRoleId = user.employmentDetails.role?._id;
  const userDepartmentId = user.employmentDetails.department?._id;
  
  // Find pending approval instances where:
  // 1. The current step is assigned to the user's role, or
  // 2. The current step is assigned to the user's department and user is a manager, or
  // 3. The step is delegated to this user
  const pendingApprovals = await ApprovalInstance.find({
    status: 'Pending',
    $or: [
      // Role matches
      { 'stepHistory.role': userRoleId, 'stepHistory.status': 'Pending' },
      // Department matches and user is a manager
      { 
        'stepHistory.department': userDepartmentId, 
        'stepHistory.status': 'Pending'
      },
      // Delegated to this user
      { 'stepHistory.delegatedTo': new mongoose.Types.ObjectId(userId) }
    ]
  })
  ...
  // Filter to include only instances where user is responsible for the current step
  return pendingApprovals.filter(instance => {
    const currentStepIndex = instance.currentStep;
    const currentStep = instance.stepHistory[currentStepIndex];
    ...
    if (
      currentStep.role && 
      userRoleId && 
      currentStep.role.toString() === userRoleId.toString()
    ) {
      return true;
    }
    
    // Check by department (if user is a manager)
    if (
      currentStep.department && 
      userDepartmentId && 
      currentStep.department.toString() === userDepartmentId.toString() && 
